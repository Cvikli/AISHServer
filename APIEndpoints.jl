OK(data) = HTTP.Response(200, data)
...
HTTP.register!(ROUTER, "POST", "/api/set_path", req -> begin
    ...
    return OK(json(Dict("status" => "success", "message" => "Project path set")))
end)
...
HTTP.register!(ROUTER, "POST", "/api/update_system_prompt", req -> begin
    ...
    return OK(json(Dict("status" => "success", "message" => "System prompt updated")))
end)

HTTP.register!(ROUTER, "GET", "/api/refresh_project", () -> begin
    update_system_prompt!(ai_state)
    return OK(json(Dict("status" => "success", "message" => "System prompt refreshed")))
end)

HTTP.register!(ROUTER, "POST", "/api/new_conversation", () -> begin
    new_id = generate_new_conversation(ai_state)
    return OK(json(Dict("status" => "success", "message" => "New conversation started", "conversation_id" => new_id)))
end)
HTTP.register!(ROUTER, "POST", "/api/select_conversation", req -> begin
    ...
    return OK(json(Dict("status" => "success", "message" => "Conversation selected and loaded", "history" => conversation_to_dict(ai_state))))
end)
...
HTTP.register!(ROUTER, "POST", "/api/process_message", req -> begin
    ...
    return OK(json(Dict("status" => "success", "response" => msg.content, 
